...
// ssbump
...
#if CASCADED_SHADOW
	float3x3 tangentSpaceTranspose = float3x3( i.tangentSpaceTranspose_Row0.xyz, i.tangentSpaceTranspose_Row1.xyz, i.tangentSpaceTranspose_Row2.xyz );
	float3 worldSpaceNormal = mul( vNormal.xyz, tangentSpaceTranspose );
#elif CUBEMAP || LIGHTING_PREVIEW || ( defined( _X360 ) && FLASHLIGHT )
	...
	float flShadow = 1.0 - DoCascadedShadow( ShadowDepthSampler, RandRotSampler, worldSpaceNormal,
		g_CascadedForward, closePosition, i.worldPos_projPosZ.xyz, 0, g_CascadedStepData,
		vProjPos.xy / vProjPos.w, float4( 0.0005, 0.0, 0.0, 0.0 ) );
