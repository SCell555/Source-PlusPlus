...
void CNPC_Barnacle::PullEnemyTorwardsMouth( bool bAdjustEnemyOrigin )
{
	...
	float flPull = fabsf(sin( m_flLocalTimer * 5 ));
...
void CNPC_Barnacle::WaitTillDead ( void )
{
	...
	if ( fabsf(flDist - goalAltitude) > 20.0f )
	{
		float flNewAltitude;
		float dt = gpGlobals->curtime - GetLastThink();
	...
	else
	{
		...
		if ( vecVelocity.LengthSqr() < 1.0f )
		{
			...
			m_hTongueTip->m_pSpring->SetSpringConstant( BARNACLE_TONGUE_SPRING_CONSTANT_HANGING );
			if ( fabsf(flDist - goalAltitude) > 1.0f )
			{
			...
			{
				bTongueInPosition = ( fabsf(flDist - goalAltitude) <= 1.0f );
			}
		}
	}
...
CBaseEntity *CNPC_Barnacle::TongueTouchEnt ( float *pflLength )
{
	...
	// Pull it up a tad
	length = MAX(8.f, length - m_flRestUnitsAboveGround);
	if ( pflLength )
	{
		*pflLength = length;
	}
...
#ifdef HL2_EPISODIC
				length = fabsf( GetAbsOrigin().z - pTest->WorldSpaceCenter().z );
				// Pull it up a tad
				length = MAX(8.f, length - m_flRestUnitsAboveGround);
				if ( pflLength )
				{
