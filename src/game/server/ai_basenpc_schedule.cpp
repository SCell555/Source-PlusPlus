...
bool CAI_BaseNPC::FindCoverPos( CSound *pSound, Vector *pResult )
{
	if ( !GetTacticalServices()->FindCoverPos( pSound->GetSoundReactOrigin(),
												...
												MIN( pSound->Volume(), 120 ),
												CoverRadius(),
												pResult ) )
	{
		return GetTacticalServices()->FindLateralCover( pSound->GetSoundReactOrigin(), MIN( pSound->Volume(), 60 ), pResult );
	}
	...
	case TASK_FACE_PATH:
		{
			...
			else
			{
				...
				if( fabsf( GetMotor()->DeltaIdealYaw() ) <= NPC_TRIVIAL_TURN )
				{
	...
	case TASK_GET_PATH_TO_TARGET_WEAPON:
		{
			...
			else
			{
				...
				float flDistZ;
				flDistZ = fabsf( vecNodePos.z - m_hTargetEnt->GetAbsOrigin().z );
	...
	case TASK_MOVE_TO_GOAL_RANGE:
		{
			...
			else
			{
				...
				distance = fabsf(vecGoalPos.z - GetLocalOrigin().z);
			...
			case 0:
				{
					...
					AngleVectors( ang, &move );
#endif	//HL2_EPISODIC
					if ( GetNavigator()->SetVectorGoal( move, pTask->flTaskData, MIN(36.f,pTask->flTaskData), true ) && IsValidMoveAwayDest( GetNavigator()->GetGoalPos() ))
					{
						TaskComplete();
					}
					else
					{
						...
						if ( GetNavigator()->SetVectorGoal( move, pTask->flTaskData, MIN(24.f,pTask->flTaskData), true ) && IsValidMoveAwayDest( GetNavigator()->GetGoalPos() ) )
						{
							TaskComplete();
						}
						else
			...
			case 1:
				{
					...
					else
					{
						...
						if ( GetNavigator()->SetVectorGoal( move, pTask->flTaskData, MIN(6.f,pTask->flTaskData), false ) && IsValidMoveAwayDest( GetNavigator()->GetGoalPos() ) )
						{
							TaskComplete();
						}
						else
...
void CAI_BaseNPC::SetTurnActivity ( void )
{
	...
	*/
	...
	if( fabsf( flYD ) >= 160 && SelectWeightedSequence ( ACT_180_LEFT ) != ACTIVITY_NOT_AVAILABLE )
	{
		Remember( bits_MEMORY_TURNING );
